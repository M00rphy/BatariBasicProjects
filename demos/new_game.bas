   ;***************************************************************
   ;
   ;  Fake Gravity Platformer Test With Shooting
   ...
   dim _Generic_Counter = o

   ;```````````````````````````````````````````````````````````````
   ;  How long player must stand while shooting.
   ;
   dim _Shot_Stand_Counter = r
   ...
   ;
   dim _BitOp_02 = x
   dim _Bit0_Shoot_Left = x            ; Player shoots left if on.
   dim _Bit1_Shoot_Right = x           ; Player shoots right if on.
   dim _Bit2_Shot_Moving = x           ; Shot is moving if on.
   dim _Bit3_Mem_Left = x              ; Remembers if player was facing left.
   dim _Bit4_Mem_Right = x             ; Remembers if player was facing right.
   dim _Bit5_Slide_Fire_Restrainer = x ; Player can't hold down fire button to repeatedly shoot.
...
__Start_Restart
   ...
   ;***************************************************************
   ;
   ;  Player shoots left if ducking right away.
   ;
   _Bit3_Mem_Left{3} = 1 
   ...
   ;***************************************************************
   ;
   ;  Makes ball 1 pixel high and 1 pixel wide.
   ;
   ballheight = 0 : CTRLPF = $01
...
__Skip_Frame_Counter
   ...
   ;```````````````````````````````````````````````````````````````
   ;  If not in a jump yet and player is ducking, don't jump.
   ;  (This is needed because the player shoots while ducking.)
   ;
   if !_Jump_Gravity_Counter && _Bit6_Duck_in_Progress{6} then goto __Skip_Jump
...
__Skip_Fall_02
   ...
   ;```````````````````````````````````````````````````````````````
   ;  Skips shot movement if shot moving bit is off.
   ;
   if !_Bit2_Shot_Moving{2} then goto __Skip_Duck_Shot_Moving

   ;```````````````````````````````````````````````````````````````
   ;  Makes player stand for a certain amount of time.
   ;
   if _Shot_Stand_Counter < 13 then goto __Skip_Duck_Fire

   goto __Skip_Duck

__Skip_Duck_Shot_Moving

   ;```````````````````````````````````````````````````````````````
   ;  Remembers player1 direction.
   ;
   _Bit0_Shoot_Left{0} = _Bit3_Mem_Left{3} : _Bit1_Shoot_Right{1} = _Bit4_Mem_Right{4}

   ;```````````````````````````````````````````````````````````````
   ;  If fire button not pressed, skip this section.
   ;
   if !joy0fire then _Bit5_Slide_Fire_Restrainer{5} = 0 : goto __Skip_Duck

   ;```````````````````````````````````````````````````````````````
   ;  If slide fire restrainer is on, don't shoot.
   ;
   if _Bit5_Slide_Fire_Restrainer{5} then goto __Skip_Duck

   ;```````````````````````````````````````````````````````````````
   ;  Sets up variables for shooting.
   ;
   _Bit2_Shot_Moving{2} = 1 : _Shot_Stand_Counter = 0 : _Bit5_Slide_Fire_Restrainer{5} = 1 

__Skip_Duck_Fire

   ;```````````````````````````````````````````````````````````````
   ;  Skips section if player moving left or right.
   ;
   if joy0left || joy0right then goto __Skip_Duck

   ;```````````````````````````````````````````````````````````````
   ;  Color and sprite data for standing to shoot.
   ;
   player1color:
   $26
   $9C
   ...
   ;```````````````````````````````````````````````````````````````
   ;  Skips section if player standing to shoot.
   ;
   if _Shot_Stand_Counter > 0 then goto __Skip_Duck

   ;```````````````````````````````````````````````````````````````
   ;  Sets up shooting sound effect.
   ;
   _Ch0_Sound = _c_Shot_Sound : _Ch0_Duration = 1 : _Ch0_Counter = 0

   ;```````````````````````````````````````````````````````````````
   ;  Shoots to the left if player was facing left.
   ;
   if _Bit0_Shoot_Left{0} then ballx = player1x + 3 : bally = player1y-8 : _Bit4_Flip_P1{4} = 0

   ;```````````````````````````````````````````````````````````````
   ;  Shoots to the right if player was facing right.
   ;
   if _Bit1_Shoot_Right{1} then ballx = player1x + 6 : bally = player1y-8 : _Bit4_Flip_P1{4} = 1
...
__Skip_Duck
   ...
   ;
   ;```````````````````````````````````````````````````````````````
   ;  Skips section if shot not moving.
   ;
   if !_Bit2_Shot_Moving{2} then goto __Shot_Not_Moving

   ;```````````````````````````````````````````````````````````````
   ;  Adds one to stand counter if stand limit not reached.
   ;
   if _Shot_Stand_Counter < 14 then _Shot_Stand_Counter = _Shot_Stand_Counter + 1

   ;```````````````````````````````````````````````````````````````
   ;  Moves ball to the left if shoot left bit on.
   ;
   if _Bit0_Shoot_Left{0} then ballx = ballx - 2

   ;```````````````````````````````````````````````````````````````
   ;  Moves ball to the right if shoot right bit on.
   ;
   if _Bit1_Shoot_Right{1} then ballx = ballx + 2

   ;```````````````````````````````````````````````````````````````
   ;  Removes shot if it hits the side of the screen.
   ;
   if ballx < 3 || ballx > 159 then bally = 250 : _Bit2_Shot_Moving{2} = 0
...
__Shot_Not_Moving
   ...
   if joy0left then gosub __Slow_Scroll_right bank2
...
__Skip_Slide_Check_Left
   ...
   ;```````````````````````````````````````````````````````````````
   ;  Turns on shooting restrainer.
   ;
   _Bit5_Slide_Fire_Restrainer{5} = 1

   ;```````````````````````````````````````````````````````````````
   ;  Remembers last direction player was facing.
   ;
   _Bit3_Mem_Left{3} = 1 : _Bit4_Mem_Right{4} = 0
...
__Skip_Joy0Left
   ...
   if joy0right then gosub __Slow_Scroll_left bank2
...
__Skip_Slide_Check_Right
   ...
   ;  Remembers last direction player was facing.
   ;
   _Bit3_Mem_Left{3} = 0 : _Bit4_Mem_Right{4} = 1
...
__Done_Anim_jump
   ...
   ;
   if joy0right || joy0left then _Slide_Counter = 0 : _Slide_Speed = 1 : goto __Check_Shot_in_Slide
...
__Check_Shot_in_Slide

   ;```````````````````````````````````````````````````````````````
   ;  Turns on shot restrainer if shot is not moving.
   ;
   if !_Bit2_Shot_Moving{2} then _Bit5_Slide_Fire_Restrainer{5} = 0
...
__Skip_Ch0_Sound_003
   ...
   ;
   if _Ch0_Sound <> _c_Shot_Sound then goto __Skip_Ch0_Sound_004

   ;```````````````````````````````````````````````````````````````
   ;  Retrieves first part of channel 0 data.
   ;
   temp4 = _SD_Shot[_Ch0_Counter]
   ...
   ;
   _Ch0_Counter = _Ch0_Counter + 1
   temp5 = _SD_Shot[_Ch0_Counter] : _Ch0_Counter = _Ch0_Counter + 1
   temp6 = _SD_Shot[_Ch0_Counter] : _Ch0_Counter = _Ch0_Counter + 1
   ...
   ;  Sets duration.
   ;
   _Ch0_Duration = _SD_Shot[_Ch0_Counter] : _Ch0_Counter = _Ch0_Counter + 1
