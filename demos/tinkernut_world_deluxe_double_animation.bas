   ...
   ;****************************************************************
   ;
   ;  Standard kernel options. Cost: loss of ball.
   ;
...
__Acorn_Frame_Done
   ...
   ;
   ;```````````````````````````````````````````````````````````````
   ;  Jumps to fire button check if ball is off the screen.
   ;
...
__AP_Fire
   ...
   ;
   bally = player0y - 2 : ballx = player0x + 4
...
__Skip_Joy_Movement
   ...
   ;***************************************************************
   ;
   ;  Squirrel/ball collision check.
   ;
   ;```````````````````````````````````````````````````````````````
   ;  Skips section if there is no collision.
   ;
   if !collision(ball,player1) then goto __Skip_Squirrel_Kill
   ...
   ;
   player1x = (rand/2) + (rand&15) : player1y = 0 : bally = 250
